...
def evaluate(model, data, mask, device):
    model.eval()
    logits, emb = model(data.x.to(device), data.edge_index.to(device))
    ...
    return pr_auc_, f1, probs, y_true, emb.numpy()
...
def train():
    ...
    data_edge_index_dev = data.edge_index.to(device)
    data_y_dev = data.y.to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        optimizer.zero_grad()
        logits, _ = model(data_x_dev, data_edge_index_dev)
        loss = criterion(logits[train_mask], data_y_dev[train_mask])
        loss.backward()
