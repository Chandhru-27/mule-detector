...
class AMLProductionEngine:
    ...
    def process_batch(
        ...
        sent_txns = txn_df.groupby("sender_account_id")["transaction_id"].agg(list).to_dict()
        recv_txns = txn_df.groupby("receiver_account_id")["transaction_id"].agg(list).to_dict()
        txn_ids_by_account = {}
        for acc in set(sent_txns.keys()).union(recv_txns.keys()):
            txn_ids_by_account[acc] = sent_txns.get(acc, []) + recv_txns.get(acc, [])
