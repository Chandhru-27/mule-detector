...
def main():
    ...
    print(f"    → {txn_path}")
    # User requested dropping labels to let the pipeline predict them
    df_unlabelled = df.drop(columns=["risk_label", "pattern_family", "cluster_id"], errors="ignore")
    df_unlabelled.to_csv(txn_path, index=False)
    ...
    with open(report_path, "w") as f:
        ...
        f.write("  OK Total cluster count matches target\n")
        f.write(f"  OK Hybrid ratio = {HYBRID_COUNT/SUSPICIOUS_TOTAL*100:.1f}% (≥35%)\n")
        f.write("  OK All rows have split assignment\n")
        f.write("  OK No cluster leakage (split assigned at cluster level)\n")
        f.write("\nOutput files:\n")
        f.write(f"  {txn_path}\n")
